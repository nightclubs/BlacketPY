...
legacy_url = 'https://blacket.org/'; session = requests.Session()
...
class BlacketPY:
    def on_connect(auth):
     try:
        connect_res = session.get(
            legacy_url + 'worker/user/getusername.php',
            cookies = {
                'PHPSESSID': auth
            },
            headers = {
        ...
        token_res = session.get(
            legacy_url + 'worker/user/gettokens.php',
            cookies = {
                'PHPSESSID': auth
            },
            headers = {
        ...
        reward_res = session.get(
            legacy_url + 'worker/user/canclaimrewards.php',
            cookies = {
                'PHPSESSID': auth
            },
            headers = {
        ...
        return '%s | %s | %s' % (connect_res.text, reward_res.text, token_res.text)
    ...
    def login(user, password):
     try:
        ...
        if 'SUCCESS' in login_res.text:
            return 'successfully logged in | PHPSESSID: %s' % login_res.cookies['PHPSESSID'] # returns login cookie as well.
    ...
    def buy_boxes(auth):
     try:
        ...
        if box in box_res.text:
            ...
            return 'Successfully bought %s box and received %s (%s)!' % (box, box_type, box_rarity)
